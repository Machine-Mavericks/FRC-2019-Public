/*----------------------------------------------------------------------------*/
/* Copyright (c) 2017-2018 FIRST. All Rights Reserved.                        */
/* Open Source Software - may be modified and shared by FRC teams. The code   */
/* must be accompanied by the FIRST BSD license file in the root directory of */
/* the project.                                                               */
/*----------------------------------------------------------------------------*/

#include "commands/HomeTilt.h"
#include "Robot.h"
#include "subsystems/IntakeTilt.h"

HomeTilt::HomeTilt() {
    // Use Requires() here to declare subsystem dependencies
  Requires (&Robot::m_IntakeTilt);

  // tank drive command is interruptable
  SetInterruptible(false);

  // command is not to run when robot is disabled
  SetRunWhenDisabled(false);
  
}

// Called just before this Command runs the first time
void HomeTilt::Initialize() {
  // initialize for homing
  Robot::m_IntakeTilt.InitForHoming();
  // turn on motor
  Robot::m_IntakeTilt.SetTiltMotorSpeed(-0.1);

}

// Called repeatedly when this Command is scheduled to run
void HomeTilt::Execute() {}

// Make this return true when this Command no longer needs to run execute()
bool HomeTilt::IsFinished() {   
  return Robot::m_IntakeTilt.IsRevLimitSwitchClosed(); 
}

// Called once after isFinished returns true
void HomeTilt::End() {

  // stop motor
  Robot::m_IntakeTilt.SetTiltMotorSpeed(0);

  //reset encoder
  Robot::m_IntakeTilt.InitPositionControl();
}

// Called when another command which requires one or more of the same
// subsystems is scheduled to run
void HomeTilt::Interrupted() {}
